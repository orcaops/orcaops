 ...
 */
describe('artifact semantics readPlanRevision', () => {
    ...
    await semantics.revisePlan(
      {
        ...
        plan_steps: [
          ...
          { step_id: STEP_B, text: 'step b text', label: 'step-b', acceptance_criteria: [] },
          {
            text: 'step c text',
            label: 'step-c',
            // A step added by a revision needs a rubric; the pre-existing
            // rubric-free step-a / step-b carry forward untouched.
  ...
  it('accumulates plan decisions across revisions (append-only, monotonic revision_n, per-revision readback)', async () => {
    ...
    const latest = await semantics.readPlan(artifactId);
    ...
    const rev0 = await semantics.readPlanRevision(artifactId, 0);
    expect(rev0?.decisions.map((d) => d.decision)).toEqual([
      'imperative in-transaction enqueueCommand',
    ]);
    const rev1 = await semantics.readPlanRevision(artifactId, 1);
  ...
  it('keeps the pinned source plan immutable across a plan revise', async () => {
    ...
    const artifactJson = await semantics.readArtifact(artifactId);
  ...
  it('does NOT finalize on pre_pr_checked — plan revise still succeeds (decoupling)', async () => {
    await writeInitialPlan();
    await semantics.writePrePrChecked(artifactId, passingPrePrReview('sha-prepr'));
  ...
  it('returns null when revision_n is negative', async () => {
    await writeInitialPlan();
    expect(await semantics.readPlanRevision(artifactId, -1)).toBeNull();
  });

  it('returns null when revision_n exceeds the latest revision', async () => {
    await writeInitialPlan();
    expect(await semantics.readPlanRevision(artifactId, 1)).toBeNull();
    expect(await semantics.readPlanRevision(artifactId, 99)).toBeNull();
  });

  it('returns the initial plan when revision_n=0 on a freshly captured artifact', async () => {
    await writeInitialPlan();
    const result = await semantics.readPlanRevision(artifactId, 0);
  ...
  it('returns the prior revision after a revise (revision_n=0 carries the initial plan shape)', async () => {
    ...
    const prior = await semantics.readPlanRevision(artifactId, 0);
  ...
  it('returns the latest revision shape when revision_n equals latest (short-circuits to readPlan)', async () => {
    ...
    const latest = await semantics.readPlanRevision(artifactId, 1);
  ...
  it('survives two revisions and returns each prior revision intact', async () => {
    ...
    const rev0 = await semantics.readPlanRevision(artifactId, 0);
    const rev1 = await semantics.readPlanRevision(artifactId, 1);
    const rev2 = await semantics.readPlanRevision(artifactId, 2);
    const rev3 = await semantics.readPlanRevision(artifactId, 3);
  ...
  it('idempotency: re-omitting a minted criterion replays; changing criterion text conflicts', async () => {
    ...
    const first = await semantics.revisePlan(reviseInput, {
    ...
    const replay = await semantics.revisePlan(reviseInput, {
      idempotencyKey: 'revise-add-criterion',
    });
    expect(replay.outcome).toBe('replay');
    ...
    const conflict = await semantics.revisePlan(
      {
        ...reviseInput,
        plan_steps: [
          {
