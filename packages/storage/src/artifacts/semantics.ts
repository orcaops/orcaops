...
import type { ArtifactMutationDraft } from './mutation-draft.js';
import type {
  AutoMintWriteOptions,
  CaptureWriteOptions,
  CheckpointAbandonInput,
  CheckpointAbandonWriteOptions,
  CheckpointAbandonWriteResult,
  CheckpointCloseInput,
  CheckpointCloseWriteOptions,
  CheckpointCloseWriteResult,
  CheckpointOpenInput,
  CheckpointOpenWriteOptions,
  CheckpointOpenWriteResult,
  EvaluatorRunWriteResult,
...
import { type AppendEventInput, type EventRecord } from '../events/event-log.js';
...
export abstract class ArtifactSemantics {
  abstract readonly repoRoot: string;
  abstract readonly store: Store;
  protected abstract readonly preparation?: ArtifactMutationDraft;
  protected abstract artifactPaths(artifactId: string): ArtifactPaths;
  protected abstract writeProjection(
    file: string,
    content: string | Buffer,
    root: string
  ): Promise<void>;
  protected abstract writeArtifactProjection(
    file: string,
    value: ArtifactJson,
    root: string
  ): Promise<void>;
  protected abstract withOperationalProjectionWrite<T>(
    root: string,
    write: () => T | Promise<T>
  ): Promise<T>;
  protected abstract withWriteLock<T>(artifactId: string, fn: () => Promise<T>): Promise<T>;
  protected abstract appendAndMirror(
    input: AppendEventInput,
    paths: ArtifactPaths
  ): Promise<EventRecord>;
  protected abstract loadAllEvents(
    eventLogPath: string,
    sidecarsDir: string,
    artifactId: string
  ): Promise<EventWithPayload[]>;
  protected abstract loadAllEventsTolerant(
    eventLogPath: string,
    sidecarsDir: string
  ): Promise<EventWithPayload[]>;
  abstract readPlan(artifactId: string): Promise<Plan | null>;
  abstract readCheckpointsRecovered(artifactId: string): Promise<Checkpoint[]>;
  abstract readSummary(artifactId: string): Promise<Summary | null>;
  abstract readEvaluatorLog(artifactId: string): Promise<EvaluatorLog | null>;
  abstract readArtifact(artifactId: string): Promise<ArtifactJson | null>;

  protected projectEvaluatorRunIntoCache(materialized: EvaluatorLog['runs'][number]): void {
  ...
  protected projectEvaluatorDispositionIntoCache(
    materialized: EvaluatorLog['dispositions'][number]
  ): void {
  ...
  protected seedPreparation(): void {
    const events = this.preparation!.read();
    for (let index = 0; index < events.length; index++) {
      if (
        !['plan_captured', 'plan_revised', 'git_import_enriched'].includes(
          events[index].record.type
        )
      )
        continue;
      const prefix = events.slice(0, index + 1);
      const plan = rebuildPlanFromEvents(prefix);
      const artifact = rebuildArtifactJsonFromEvents(prefix);
      if (plan && artifact)
        this.projectPlanIntoCache(
          plan.plan,
          latestPlanRevisionEventId(prefix) ?? plan.sourceEventId,
          artifact.json.branch_lineage
        );
    }
    for (const checkpoint of rebuildAllCheckpointsFromEvents(events))
      this.store.upsertCheckpoint(checkpointToRow(checkpoint));
    const summary = rebuildSummaryFromEvents(events);
    const artifact = this.store.getArtifact(this.preparation!.artifactId);
    if (summary && artifact) this.projectSummaryIntoCache(summary.summary, artifact);
    const log = rebuildEvaluatorLogFromEvents(events, this.preparation!.artifactId)?.log;
    for (const run of log?.runs ?? []) this.projectEvaluatorRunIntoCache(run);
    for (const disposition of log?.dispositions ?? [])
      this.projectEvaluatorDispositionIntoCache(disposition);
  }
   ...
   */
  protected async repairCheckpointProjection(
    paths: ArtifactPaths,
    ...
    if (rowTorn) {
      ...
      if (rebuiltArtifact) {
        await this.writeArtifactProjection(paths.artifactJson, rebuiltArtifact.json, this.repoRoot);
      }
      this.store.upsertCheckpoint(checkpointToRow(checkpoint));
    }
   ...
   */
  protected projectPlanIntoCache(
    plan: Pick<
      PlanInput,
      | 'artifact_id'
      | 'branch'
      ...
      | 'origin'
    > &
      Partial<Pick<Plan, 'revision_n' | 'revised_at' | 'rationale' | 'prior_plan_event_id'>>,
    ...
    this.store.upsertPlanRevision({
      plan: {
        ...
        prior_event_id: plan.prior_plan_event_id ?? null,
  ...
  async writePlan(input: PlanInput, opts: PlanWriteOptions = {}): Promise<{ event_id: string }> {
    ...
    const paths = this.artifactPaths(plan.artifact_id);
    ...
    return this.withWriteLock(plan.artifact_id, async () => {
      ...
      if (priorPlan) {
        ...
        if (rebuilt && rebuiltJson) {
          ...
          await this.writeProjection(paths.planMd, planMarkdown(rebuilt.plan), this.repoRoot);
          await this.writeArtifactProjection(paths.artifactJson, rebuiltJson.json, this.repoRoot);
          this.projectPlanIntoCache(
            rebuilt.plan,
      ...
      await this.writeProjection(paths.planMd, planMarkdown(rebuiltPlan.plan), this.repoRoot);
      await this.writeArtifactProjection(paths.artifactJson, rebuiltArtifact.json, this.repoRoot);
  ...
  async hasCommittedPlanCapture(artifactId: string, idempotencyKey: string): Promise<boolean> {
    const paths = this.artifactPaths(artifactId);
    const events = await this.loadAllEvents(
      paths.eventsNdjson,
      paths.sidecarsDir,
      paths.artifactId
  ...
  async revisePlan(
    ...
    const paths = this.artifactPaths(input.artifact_id);
    ...
    return this.withWriteLock(input.artifact_id, async () => {
      ...
      const recordAndThrow = async (err: Error): Promise<never> => {
        await this.withOperationalProjectionWrite(this.repoRoot, () =>
          recordHardRejected({
            store: this.store,
            artifactId: input.artifact_id,
            idempotencyKey: opts.idempotencyKey,
  ...
  async writeCheckpointClosed(
    ...
    return this.withWriteLock(input.artifact_id, async () => {
      ...
      // Only within-artifact event order proves intervals. Supplied cross-artifact
      // observations and the legacy wall-clock scan remain claims-only evidence.
      ...
      const crossArtifactSiblings = opts.skipWallClockOverlapScan
        ? []
        : opts.crossArtifactSiblings !== undefined
          ? z
              .array(z.object({ artifact_id: z.string().min(1), n: z.number().int().positive() }))
              .parse(opts.crossArtifactSiblings)
          : this.store
   ...
   */
  protected projectSummaryIntoCache(summary: Summary, artifact: ArtifactRow): void {
  ...
  async writeSummary(
    ...
    const paths = this.artifactPaths(s.artifact_id);
    const idempotencyKey = opts.idempotencyKey ?? uuidv7();
    ...
    return this.withWriteLock(s.artifact_id, async () => {
      ...
      if (replay.kind === 'replay') {
        ...
        if (cacheTorn) {
          ...
          await this.writeProjection(paths.summaryMd, summaryMarkdown(priorSummary), this.repoRoot);
          if (rebuiltArtifact) {
            await this.writeArtifactProjection(
              paths.artifactJson,
              rebuiltArtifact.json,
              this.repoRoot
            );
          }
        }
  ...
  async writeEvaluatorRunPayload(
    ...
    return this.withWriteLock(artifactId, async () => {
      ...
      this.projectEvaluatorRunIntoCache(materialized);
      ...
      return {
        outcome: this.preparation?.isReplay(event.event_id)
          ? ('replay' as const)
          : ('created' as const),
        log: rebuiltLog.log,
      };
  ...
  async writeEvaluatorDisposition(
    ...
    return this.withWriteLock(artifactId, async () => {
      ...
      this.projectEvaluatorDispositionIntoCache(materialized);
  ...
  async appendBranchLineage(
    ...
    return this.withWriteLock(artifactId, async () => {
      ...
      await this.writeArtifactProjection(paths.artifactJson, rebuilt.json, this.repoRoot);
      ...
      return {
        outcome: this.preparation?.isReplay(event.event_id) ? 'replay' : 'created',
  ...
  protected async loadSummaryFromEvents(events: EventWithPayload[]): Promise<Summary | null> {
    const r = rebuildSummaryFromEvents(events);
    return r ? r.summary : null;
  }
}
export function assertCheckpointStepClaimsDisjoint(
...
export function extractSourceEventId(json: unknown): string | null {
  if (typeof json !== 'object' || json === null) return null;
  const v = (json as { source_event_id?: unknown }).source_event_id;
  return typeof v === 'string' && v.length > 0 ? v : null;
}
export function normalizePriorPayload(
...
export function checkpointToRow(cp: Checkpoint): CheckpointRow {
  if (cp.status === 'open') {
    return {
      status: 'open',
...
export function extractInputStepShapeFromCommitted(payload: Record<string, unknown>): Array<{
  step_id: string | null;
  text: string;
  label: string;
...
export function extractAckDrops(payload: Record<string, unknown>): string[] {
  const v = payload.acknowledge_drops_completed_steps;
  if (!Array.isArray(v)) return [];
  return [...v.filter((x): x is string => typeof x === 'string')].sort();
}
export function extractAckCriteriaChanges(payload: Record<string, unknown>): string[] {
  const v = payload.acknowledge_criteria_changes;
  if (!Array.isArray(v)) return [];
  return [...v.filter((x): x is string => typeof x === 'string')].sort();
}
export function resolveRevisionAgentSessionForReplay(
...
export function extractNewDecisionsFromCommitted(
...
export function validateWarningAcceptance(
  artifactId: string,
...
export function acceptedWarningsEqual(
