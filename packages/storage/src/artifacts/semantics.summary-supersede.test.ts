...
describe('writeSummary — supersede gate', () => {
  ...
  it('refuses to amend when the retained summary fails SummarySchema', async () => {
    const first = await store.writeSummary(summaryInput({ head_sha: 'head-A' }), {
      idempotencyKey: 'sum-head-malformed',
    });
    const retained = draft.events.find(
      (event) =>
        event.record.event_id === first.event_id && event.record.type === 'summary_captured'
    );
    expect(retained).toBeDefined();
    delete (retained!.payload as { head_sha?: unknown }).head_sha;

    await expect(
      store.writeSummary(summaryInput({ outcome: 'amended', head_sha: 'head-CURRENT' }), {
        idempotencyKey: 'sum-head-malformed-2',
        priorSummaryEventId: first.event_id,
      })
    ).rejects.toThrow(/head_sha/);

    expect(draft.events.filter((event) => event.record.type === 'summary_captured')).toHaveLength(
      1
    );
    expect(JSON.stringify(draft.events)).not.toContain('head-CURRENT');
  });
