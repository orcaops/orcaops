...
function removeFocusRows(file: string, operationId?: string) {
  ...
  if (operationId)
    db.prepare('DELETE FROM execution_focus_records WHERE operation_id=?').run(operationId);
  else db.exec('DELETE FROM execution_focus_current; DELETE FROM execution_focus_records');
  for (const trigger of triggers) db.exec(trigger.sql);
  db.close();
}
it('does not hide missing original focus behind a later healthy clear or poison another scope', async () => {
  const f = await focusFixture();
  const cleared = await publishProjectExecutionFocus(f.handle, {
    action: 'clear',
    operationId: uuidv7(),
    scope: f.change.scope,
    expectedSelection: f.publication.value.selection,
    secretAllow: [],
  });
  removeFocusRows(f.file, f.change.operationId);
  ...
  await expect(publishProjectExecutionFocus(f.handle, f.change)).rejects.toMatchObject({
    code: 'HISTORY_INTEGRITY_REQUIRED',
  });
  await expect(
    publishProjectExecutionFocus(f.handle, {
      action: 'clear',
      operationId: uuidv7(),
      scope: f.change.scope,
      expectedSelection: cleared.value.selection,
      secretAllow: [],
    })
  ...
  const other = {
    ...f.change.scope,
    shellKey: { kind: 'codex_session' as const, value: 'another-session' },
  };
  expect(readProjectExecutionFocus(f.handle, other).status).toBe('absent');
  const independent = await publishProjectExecutionFocus(f.handle, {
    action: 'clear',
    operationId: uuidv7(),
    scope: other,
    expectedSelection: null,
  ...
  expect(independent.value.status).toBe('cleared');
});
it('refuses the shared task focus selector after receipt-backed row loss without repair', async () => {
  const f = await focusFixture();
  const input = { branch: 'target', focusScope: f.change.scope };
  expect(readProjectTaskContext(f.handle, input).focus?.status).toBe('present');
  removeFocusRows(f.file);
  const before = saved(f.handle);
  expect(() => readProjectTaskContext(f.handle, input)).toThrow(
    expect.objectContaining({ code: 'HISTORY_INTEGRITY_REQUIRED' })
  );
  expect(saved(f.handle)).toEqual(before);
});
