...
/**
 * First entry of a comma-separated config-dir variable, `~` expanded. Matches
 * `claudeProjectBases` in `@orcaops/agent-activity`; a separator-only or empty
 * value falls back to the default.
 */
export function firstConfiguredDir(raw: string | undefined, fallback: string): string {
  const first = (raw ?? '')
    .split(',')
    .map((entry) => entry.trim())
    .filter(Boolean)[0];
  if (first === undefined) return fallback;
  if (first === '~') return homedir();
  if (first.startsWith('~/')) return join(homedir(), first.slice(2));
  return first;
}

const home = homedir();
// Use xdg-basedir (not env-paths) to match OpenCode/Amp/Goose behavior on all platforms.
const xdgConfig = process.env.XDG_CONFIG_HOME?.trim() || undefined;
const configHome = xdgConfig ?? join(home, '.config');
const codexHome = process.env.CODEX_HOME?.trim() || join(home, '.codex');
// DEVIATION from upstream: `CLAUDE_CONFIG_DIR` is a comma-separated LIST and
// may carry an unexpanded `~`, both of which this repo already handles in
// `@orcaops/agent-activity`'s locator. Reading it raw yields a path like
// `/a,/b/skills`, or one resolved against the CWD — neither of which is a real
// root, and global-install then treats every recorded entry as belonging to
// another environment.
const claudeHome = firstConfiguredDir(process.env.CLAUDE_CONFIG_DIR, join(home, '.claude'));
