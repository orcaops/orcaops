...
async function fixture() {
  const root = await mkdtemp(path.join(tmpdir(), 'source-git-process-'));
  roots.push(root);
...
describe('source Git process lifetime', () => {
  it('reads a real repository and refuses pre-cancelled inspection before spawning', async () => {
    const f = await fixture();
    expect((await runLegacyGit(f.cwd, ['rev-parse', '--is-inside-work-tree'])).stdout.trim()).toBe(
      'true'
    );
    await expect(
      runLegacyGit(f.cwd, ['rev-parse', '--git-dir'], { signal: AbortSignal.abort() })
    ).rejects.toMatchObject({ name: 'AbortError' });
    expect(await readdir(f.root)).toEqual(['repository']);
  });
  it('cancels a suspended Git child through source discovery and waits for its exit', async () => {
    const f = await fixture();
    const started = await suspendGit(f.root);
    const controller = new AbortController();
    const pending = discoverLegacyRepository({
      cwd: f.cwd,
      root: path.join(f.root, 'history'),
