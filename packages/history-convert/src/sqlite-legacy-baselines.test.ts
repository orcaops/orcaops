...
import { decodeLegacySqlite } from './sqlite.js';

// The v20 DDL is the observed 001→020 migration product. Four retained caches share
// it; a fresh run at the v20 tip differs only in two comments embedded in evaluator_runs.
const observedVersion20Schema = `
CREATE TABLE artifacts (
  ...
  -- JSON-encoded array (see migration 006).
  non_goals    TEXT NOT NULL DEFAULT '[]'
, "cloud_synced_at" TEXT, "cloud_sync_hash" TEXT, "cloud_external_id" TEXT, "cloud_org_id" TEXT, label TEXT NOT NULL DEFAULT 'unlabelled', cloud_last_push_attempt_at TEXT, cloud_last_push_error_kind TEXT, cloud_last_push_error_message TEXT, cloud_consecutive_failures INTEGER NOT NULL DEFAULT 0);
...
CREATE TABLE checkpoints (
      ...
      ts                 TEXT, open_plan_revision_event_id TEXT,
...
CREATE TABLE evaluator_runs (
  ...
  error_message       TEXT,
  ts                  TEXT NOT NULL,
  -- Materialized disposition (target-arch §10). NULL for runs that
  ...
  -- Order-key components (target-arch §14). Strict total order
...
CREATE TABLE plans (
      ...
      prior_event_id   TEXT,
      source_event_id  TEXT NOT NULL, label TEXT NOT NULL DEFAULT 'unlabelled', criterion_lineage TEXT NOT NULL DEFAULT '{"added":[],"removed":[],"rewritten":[]}', decisions TEXT NOT NULL DEFAULT '[]',
...
CREATE INDEX idx_lineage_branches_branch
      ON lineage_branches(branch_name);

CREATE INDEX idx_lineage_by_latest_sha_sha
      ON lineage_by_latest_sha(latest_lineage_sha);

CREATE INDEX idx_llm_sessions_last_used ON llm_sessions(last_used_at DESC);

CREATE INDEX idx_plan_idempotency_artifact
      ON plan_idempotency(artifact_id);
...
`;
// Version 22 is the exact historical baseline introduced by 976e1630.
const version22Schema = `
...
`;
const lifecycleConstraint = `  CONSTRAINT checkpoints_lifecycle_fields CHECK (
    (status = 'open'
...
`;
const llmSessionsDefinition = `CREATE TABLE llm_sessions (
  artifact_id  TEXT PRIMARY KEY REFERENCES artifacts(id) ON DELETE CASCADE,
  session_id   TEXT NOT NULL,
  model        TEXT,
  opened_at    TEXT NOT NULL,
...
`;
const version23Schema = version22Schema
  .replace(
    "VALUES ('version', '22');",
    "VALUES ('version', '23');\n" +
  ...
  .replace(
    '  open_plan_revision_event_id TEXT,\n  PRIMARY KEY (artifact_id, n)',
    `  open_plan_revision_event_id TEXT,\n${lifecycleConstraint}  PRIMARY KEY (artifact_id, n)`
  )
  .replace(llmSessionsDefinition, '');
const version24Schema = version23Schema
  .replace("VALUES ('version', '23');", "VALUES ('version', '24');")
  .replace(
    '  -- prompt-cache usage; command-engine runs leave them NULL.\n  model               TEXT,',
    '  -- prompt-cache usage; command-engine runs leave them NULL.\n' +
...
function populatedDatabase(version: keyof typeof historicalSchemas) {
  const db = new Database(':memory:');
  opened.push(db);
  db.exec(historicalSchemas[version]);
  db.prepare(
    'INSERT INTO artifacts(id,branch,task,agent,base_sha,started_at,status,cloud_external_id,cloud_org_id,cloud_consecutive_failures) VALUES(?,?,?,?,?,?,?,?,?,?)'
  ).run(
    'synthetic-artifact',
    'synthetic-branch',
  ...
  db.prepare('INSERT INTO cli_session_branch_state VALUES(?,?,?,?,?,?)').run(
  ...
  db.prepare('INSERT INTO plan_idempotency VALUES(?,?,?)').run(
    'synthetic-plan-key',
    'synthetic-artifact',
  ...
  db.prepare('INSERT INTO source_plan_links VALUES(?,?,?,?)').run(
    'synthetic-plan-ref',
    'synthetic-artifact',
  ...
  db.prepare(
    'INSERT INTO usage_snapshots(snapshot_id,idempotency_key,artifact_id,source_plan_ref_id,agent,session_id,lifecycle_event,cumulative_input_tokens,cumulative_output_tokens,cumulative_cache_creation_input_tokens,cumulative_cache_read_input_tokens,baseline_kind,model_breakdown,record_count,as_of,ts,dimensions) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
  ).run(
    'synthetic-snapshot',
    'synthetic-usage-key',
    'synthetic-artifact',
    ...
    '2026-08-01T00:05:01.000Z',
    '{"web_requests":1}'
  );
  db.prepare('INSERT INTO evaluator_lifecycles VALUES(?,?,?,?)').run(
    'synthetic-artifact',
    'post-plan',
    0,
...
describe('historical SQLite baselines', () => {
  it.each([20, 22, 23, 24] as const)(
    'accepts version %s and retains its operational rows',
    (version) => {
      const db = populatedDatabase(version);
      const decoded = decodeLegacySqlite({ main: db.serialize() });
      expect(decoded.baselineVersion).toBe(version);
      expect(decoded.rows.artifacts[0]).toMatchObject({
        id: 'synthetic-artifact',
        cloud_external_id: 'synthetic-external',
        cloud_org_id: 'synthetic-org',
        cloud_consecutive_failures: 2,
      });
      expect(decoded.rows.cli_session_branch_state[0]).toMatchObject({
        current_branch: 'synthetic-branch',
        branch_history: '["former-branch"]',
      });
      expect(decoded.rows.plan_idempotency[0]?.idempotency_key).toBe('synthetic-plan-key');
      expect(decoded.rows.idempotency_blocks[0]?.payload_hash).toBe('synthetic-payload-hash');
      expect(decoded.rows.source_plan_links[0]).toMatchObject({
        source_plan_ref_id: 'synthetic-plan-ref',
        pinned_version: '7',
      });
      expect(decoded.rows.usage_snapshots[0]).toMatchObject({
        snapshot_id: 'synthetic-snapshot',
        cumulative_input_tokens: 101,
        dimensions: '{"web_requests":1}',
      });
      expect(decoded.rows.evaluator_lifecycles[0]).toMatchObject({
  ...
  it.each([20, 22, 23, 24] as const)('rejects an altered version %s shape', (version) => {
    const db = populatedDatabase(version);
    db.exec('ALTER TABLE source_plan_links ADD COLUMN unexpected_value TEXT');
