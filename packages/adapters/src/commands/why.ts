...
supporting checkpoint's files/head SHA. Use \`/${commandRef('show', prefix)} <id>\` only when
...
Use \`/${commandRef('digest', prefix)}\` only for a separately requested broader PR picture.
`,
};
