...
export const orcaopsTaskReviewSkill: SkillTemplate = {
  ...
  description:
    'Generate the two-lane Task Review for a branch — a capture-grounded account lane and a ' +
    'capture-blind forensic lane, deterministically merged — optionally associate captured ' +
    'reasoning with changed code after review finalization — or address open reviewer ' +
    'comments. Cues: "generate a review" / "generate a review using orcaops", "compose the ' +
    'review", "review the work in this branch", "anchor the review reasoning to code", and ' +
    'ADDRESS-COMMENTS — "address the open ' +
    'review comments on <branch>", "answer the reviewer", "work the review comments". Skip ' +
    'for: adversarial verification of the work itself (adversarial-review skill) or a ' +
    'shareable PR summary (digest skill).',
  tags: ['orcaops', 'review'],
  group: 'review',
  defaultEnabled: true,
  blockTriggerLine: (prefix: string) =>
    `task-review generation + comment loop ("generate a review", "compose the review", "address the open review comments") → \`${skillRef('task-review', prefix)}\``,
