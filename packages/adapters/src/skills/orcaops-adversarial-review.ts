...
export const orcaopsAdversarialReviewSkill: SkillTemplate = {
  ...
  description:
    'Cross-examine what the agent claims it did — "red-team this", "poke holes", ' +
    '"adversarial review", "verify the done criteria", "review the agent\'s code". Two ' +
    'dimensions: a CLAIMS AUDIT (refutes done_criteria evidence, attacks recorded ' +
    'uncertainty, audits criterion lineage for goalpost moves, checks non-goal violations, ' +
    'and sweeps BOTH the uncommitted diff and in-window commits for work no checkpoint ' +
    'accounts for) and an UNCERTAINTY-LED CODE REVIEW (starts from the recorded uncertainty, ' +
    'never re-flags documented decisions, flags silent divergence from them). Skip for: ' +
    'evaluator gating before a PR (pre-pr skill) or a shareable summary (digest skill).',
  tags: ['orcaops', 'review'],
  group: 'review',
  defaultEnabled: true,
