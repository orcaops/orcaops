...
/**
 * author-evaluator — walk a developer's agent through authoring an evaluator,
 * command engine or LLM engine, against the published authoring surface.
 *
 * Written as a PROCEDURE, not a reference: the docs guide keeps the depth, and
 * duplicating it here is the maintenance burden this template exists to avoid.
 * The body points at shapes rather than restating them — `makeContext()`, `eval
 * test --print-example-fixture`, `eval schema` — because a prose copy of a
 * schema goes stale silently while the schema moves.
 *
 * What the body DOES carry verbatim is the set of cross-field rules the schema
 * generator cannot recover from `superRefine` bodies. Those five are precisely
 * the boundary between "the emitted schema teaches this" and "only the skill
 * can." A contract test binds each rule to its coverage here.
 *
 * `disableModelInvocation` keeps an agent from auto-firing an authoring session
 * mid-capture; the block trigger line preserves discoverability by RECOMMENDING
 * the slash invocation to the human rather than taking it. Only Claude Code
 * enforces the field, so the body must read sensibly when loaded automatically
 * too — nothing here depends on the gate for safety.
 */
export const orcaopsAuthorEvaluatorSkill: SkillTemplate = {
  id: 'author-evaluator',
  ...
  // No `; ` inside the line: the managed block joins intent entries with one,
  // and an internal semicolon would read as a second entry. The wording
  // RECOMMENDS the invocation rather than instructing the agent to take it —
...
Related: ${'`' + skillRef('doctor', prefix) + '`'} diagnoses a pack that
