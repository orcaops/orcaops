...
/**
 * recap — the window/ref-range summarizer: three named output formats; a
 * thin wrapper over deterministic `--json` commands (house style); the
 * journal format's file append stays consent-gated and agent-performed —
 * the CLI never writes user notes files.
 */
export const orcaopsRecapSkill: SkillTemplate = {
  id: 'recap',
  name: 'Orcaops: recap (standup / changelog / journal)',
  description:
    'Summarize captured work over a time window or git ref range, in three named formats. ' +
    'Standup: "standup", "what did I do yesterday / this week", "progress report", or client ' +
    'updates. Changelog: "changelog since v1.2", "what shipped between these tags?", "draft ' +
    'the release notes" — task labels and outcomes in user-intent wording, not commit ' +
    'messages. Journal: "journal today", "update my dev log", "append to NOTES.md what ' +
    'happened this week" — the CLI stays read-only; YOU write the file. Skip for: picking a ' +
    'single task back up (resume skill), a current-branch survey (`orcaops status --json`), ' +
    "or one PR's reviewer-facing summary (digest skill).",
...
- Reviewer-facing view of one PR / one artifact → \`${skillRef('digest', prefix)}\`.
