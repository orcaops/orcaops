...
export const orcaopsPlanCritiqueSkill: SkillTemplate = {
  ...
  description:
    'Critique a plan against captured history BEFORE work starts — "critique this plan", ' +
    '"review my plan draft", "poke holes in this plan before I start", "have we solved ' +
    'something like this before?", "what did we decide last time we touched auth?" — and as a ' +
    'pre-step before `capture plan` on non-trivial work. Two paths: a prior-art sweep feeding ' +
    'the capture (decisions, rejected alternatives, non-goals, unresolved uncertainty from ' +
    'captured artifacts — every archived project when the archive is enabled), and a draft ' +
    'critique against past decisions, fragile files, abandoned attempts, and weak acceptance ' +
    'criteria. Skip for: continuing an in-flight task (resume skill).',
  tags: ['orcaops', 'insight'],
  group: 'insight',
  defaultEnabled: true,
  blockTriggerLine: (prefix: string) =>
    `plan critique / prior art before planning ("critique this plan", "have we solved this before?") → \`${skillRef('plan-critique', prefix)}\``,
