...
export const orcaopsDoctorSkill: SkillTemplate = {
  id: 'doctor',
  name: 'Orcaops: diagnose install',
  description:
    'Diagnose adapter health, env, evaluator validity, cache integrity, and watchdog signals. Use when the user asks "is orcaops set up correctly?", "what\'s broken?", "diagnose orcaops", or when an orcaops command fails unexpectedly.',
...
- \`stale-artifacts\` warns → if the work is complete, suggest the
  ${skillRef('finish', prefix)} skill; otherwise resume or amend the artifact.
