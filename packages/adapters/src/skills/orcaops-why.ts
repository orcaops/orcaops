...
Pair with the \`${commandRef('show', prefix)}\` invocation for the full artifact thread,
or \`${skillRef('digest', prefix)}\` for the broader PR picture.
...
1. Recommend the user-invoked \`${skillRef('seed', prefix)}\` skill for the
...
2. Record the offer you just made, so it is not repeated during the
   ...
   \`${skillRef('seed-discovery', prefix)}\` lane records) — it stamps only
...
(You don't yet know WHERE it broke? → \`${skillRef('timetravel', prefix)}\` bisects
