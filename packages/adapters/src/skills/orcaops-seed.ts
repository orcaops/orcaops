...
import { seedEnrichmentOrchestration } from './seed-orchestration.js';

export const orcaopsSeedSkill: SkillTemplate = {
  id: 'seed',
  name: 'Orcaops: seed git history',
  description:
    "Backfill Orcaops once from an existing repository's git history. Use when the user says " +
    '"seed orcaops from git history", "import my git history", "backfill my history", or ' +
    '"why is orcaops search empty?". Preview first, explain synthesized provenance, obtain ' +
    'confirmation, orchestrate optional enrichment, apply once, and finish the importance lane.',
...
Do not use \`${skillRef('seed-discovery', prefix)}\` for the initial one-time
backfill; that skill owns later, scoped gap filling.

${seedEnrichmentOrchestration(options)}
