// Vendored from hunk (https://github.com/modem-dev/hunk) @ 9ef9b2e, source path src/ui/diff/rowWindowing.ts
// MIT License, Copyright (c) Ben Vinegar. Full text: packages/diff-render/LICENSE.
// Adaptations for @orcaops/diff-render:
//   - the `DiffSectionGeometry` / `PlannedReviewRow` type imports (from the un-vendored
//     diffSectionGeometry / reviewRenderPlan modules — the latter permanently off-limits, see
//     ./_boundary.ts) replaced by the minimal structural stand-ins `WindowedSectionGeometry`
//     (bodyHeight + rowBounds) and a `Row` type parameter; the function bodies are verbatim.

/** Minimal measured-bounds surface the row window reads (stand-in for DiffSectionGeometry). */
export interface WindowedRowBounds {
  top: number;
  height: number;
}

export interface WindowedSectionGeometry {
  bodyHeight: number;
  rowBounds: WindowedRowBounds[];
}
...
export interface VisiblePlannedRowWindow<Row> {
 ...
 */
function findFirstRowWithBottomAfter(rowBounds: WindowedSectionGeometry['rowBounds'], top: number) {
  let low = 0;
 ...
 */
function findLastRowWithTopBefore(rowBounds: WindowedSectionGeometry['rowBounds'], bottom: number) {
  let low = 0;
  let high = rowBounds.length - 1;
  let result = -1;
...
function rowOverlapsVisibleRange(
  rowBounds: WindowedSectionGeometry['rowBounds'][number],
 ...
 */
export function resolveVisiblePlannedRowWindow<Row>({
  plannedRows,
  sectionGeometry,
  visibleBodyBounds,
}: {
  plannedRows: Row[];
  sectionGeometry: WindowedSectionGeometry;
  visibleBodyBounds: VisibleBodyBounds;
}): VisiblePlannedRowWindow<Row> {
