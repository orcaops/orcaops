// Vendored from hunk (https://github.com/modem-dev/hunk) @ 9ef9b2e, source path src/ui/diff/renderRows.tsx
// MIT License, Copyright (c) Ben Vinegar. Full text: packages/diff-render/LICENSE.
// Adaptations for @orcaops/diff-render:
//   - PlannedReviewRow / inlineNoteTitle / CopySelectedRowRange retargeted from the un-vendored reviewRenderPlan / AgentInlineNote / copySelection modules to the local ./_boundary severance shim. That boundary is PERMANENT: pins ship through DiffSlice's slot seams instead of vendoring the upstream review-render-plan splice loop, so these stand-ins are never replaced — see ./_boundary.ts.
//   - DiffRowFocus is accepted as orthogonal presentation metadata so subdued
//     out-of-checkpoint cells retain canonical diff semantics and syntax spans.
...
import type { DiffCellFocus, DiffRowFocus } from "./focusMask";
import { type PlannedReviewRow } from "./_boundary";
import { inlineNoteTitle } from "./_boundary";
...
import type { CopySelectedRowRange } from "./_boundary";
...
function renderInlineSpans(
  ...
  for (const span of trimmed) {
    ...
    const spanBg = span.bg === undefined ? fallbackBg : (emphasisBgOverride ?? span.bg);
    colPos = spanEnd;
    ...
    if (selected) {
      elements.push(
        <span
          key={`${keyPrefix}:${elementIndex++}`}
          fg={span.fg ?? fallbackColor}
          bg={selectionHighlightBg(spanBg, selectionTheme)}
        >
          {selected}
        </span>,
      );
...
interface WrappedCellLine {
  gutterText: string;
  /** The semantic +/- glyph on the first visual line; continuation gutters omit it. */
  sign?: string;
  spans: RenderSpan[];
}
...
function buildWrappedSplitCell(
  ...
  const palette = splitCellPalette(cell.kind, theme, cell.moveKind, focus);
...
function buildWrappedStackCell(
  ...
  const palette = stackCellPalette(cell.kind, theme, cell.moveKind, focus);
...
/**
 * Render line numbers and the semantic +/- glyph as separate color runs.
 *
 * `rowStyle` deliberately gives subdued rows neutral number columns but vivid
 * signs. Keeping the combined gutter in one span silently discarded signColor.
 */
function renderDiffGutter(
  gutterText: string,
  sign: string | undefined,
  palette: { gutterBg: string; numberColor: string; signColor: string },
  keyPrefix: string,
) {
  const semanticSign = sign === "+" || sign === "-" ? sign : null;
  const signIndex = semanticSign === null ? -1 : gutterText.lastIndexOf(semanticSign);
  if (semanticSign === null || signIndex < 0) {
    return (
  ...
  const before = gutterText.slice(0, signIndex);
  const after = gutterText.slice(signIndex + semanticSign.length);
  return (
    <>
      {before ? (
        <span
          key={`${keyPrefix}:gutter:number-before`}
      ...
      <span key={`${keyPrefix}:gutter:sign`} fg={palette.signColor} bg={palette.gutterBg}>
        {semanticSign}
      </span>
      {after ? (
        <span
          key={`${keyPrefix}:gutter:number-after`}
          fg={palette.numberColor}
          bg={palette.gutterBg}
        >
          {after}
...
function renderSplitCell(
  ...
  const basePalette = splitCellPalette(cell.kind, theme, cell.moveKind, focus);
  ...
  return (
    <>
      ...
      {renderDiffGutter(gutterText, cell.sign, palette, keyPrefix)}
      {renderInlineSpans(
        cell.spans,
        contentWidth,
        ...
        localColRange,
        focus === "subdued" ? subduedWordDiffBackground(cell.kind, theme) : undefined,
      )}
    </>
  );
}
...
function renderStackCell(
  ...
  const basePalette = stackCellPalette(cell.kind, theme, cell.moveKind, focus);
  const palette = selected ? applySelectionPalette(basePalette, theme) : basePalette;
  const resolvedPrefix = selected && prefix ? applySelectionPrefix(prefix, theme) : prefix;
  const prefixWidth = resolvedPrefix?.text.length ?? 0;
  const { gutterWidth, contentWidth } = resolveStackCellGeometry(
...
function renderWrappedSplitCellLine(
  ...
  return (
    <>
      ...
      {renderDiffGutter(line.gutterText, line.sign, resolvedPalette, keyPrefix)}
      {renderInlineSpans(
        line.spans,
        contentWidth,
        theme.syntaxColors.default,
...
function renderRow(
  ...
  if (row.type === "collapsed") {
    ...
    const leftFocus = focus?.kind === "split" ? focus.left : "primary";
    const rightFocus = focus?.kind === "split" ? focus.right : "primary";
    ...
    const leftPrefix = {
      text: guideOnOldSide ? "│" : marker(leftFocus),
      fg: guideOnOldSide
        ? theme.noteBorder
        : splitLeftRailColor(row.left.kind, theme, selected || hasCopySelection, leftFocus),
      bg: theme.panel,
    };
    const rightPrefix = {
      text: marker(rightFocus),
      fg: splitRightRailColor(row.right.kind, theme, selected || hasCopySelection, rightFocus),
      bg: theme.panel,
    };
    ...
    if (!wrapLines) {
      ...
      baseRow = (
        <box id={anchorId} style={{ width: "100%", height: visualLineCount, flexDirection: "column" }}>
                ...
                >
                  <text>
                    ...
                    {renderWrappedSplitCellLine(
                      ...
                      hasRightSelection ? copySelectedRowRange : undefined,
                      leftWidth,
                      rightLayout.emphasisBgOverride,
                    )}
                    {guideOnNewSide ? (
    ...
    const cellFocus = focus?.kind === "stack" ? focus.cell : "primary";
    const guideOnOldSide = noteGuideSide === "old";
    const guideOnNewSide = noteGuideSide === "new";
    const addNoteTarget: UserNoteLineTarget | undefined =
    ...
    const prefix = {
      text: guideOnOldSide ? "│" : marker(cellFocus),
      fg: guideOnOldSide
        ? theme.noteBorder
        : stackRailColor(row.cell.kind, theme, selected || hasCopySelection, cellFocus),
      bg: theme.panel,
    };
    ...
    if (!wrapLines) {
      ...
      baseRow = (
        <box id={anchorId} style={{ width: "100%", height: layout.lines.length, flexDirection: "column" }}>
...
interface DiffRowViewProps {
  ...
  theme: AppTheme;
  selected: boolean;
  /** Orthogonal ownership presentation; the canonical row remains semantically unchanged. */
  focus?: DiffRowFocus;
